...
def detect_square_point_count(point_total, fallback_point_count):
    """如果點數剛好是平方數，例如49點，就自動判斷為7×7。"""
    try:
        root = int(round(np.sqrt(point_total)))
        if root >= 3 and root * root == point_total and root % 2 == 1:
            return root
    except Exception:
        pass
    return int(fallback_point_count)


def assign_matrix_by_actual_position(result, point_count, matrix_length, auto_fit_matrix):
    """
    只依照「實際X / 實際Y」重建矩陣位置：
    1. 先依實際Y由大到小分列
    2. 每一列依實際X由小到大排序
    ...
    auto_fit_matrix=True：
        用實際值反推最接近的均勻矩陣，適合 Excel 只有實際值時使用。
        注意：這會扣掉整體縮放與整體位移，主要看局部點位偏移。

    auto_fit_matrix=False：
        使用左側「矩陣總長度」產生理論座標，適合你已知設計尺寸時使用。
    """
    expected_count = int(point_count) * int(point_count)

    if len(result) != expected_count:
        st.warning(
            f"目前讀到 {len(result)} 個實際點，但你設定/辨識的矩陣是 {point_count}×{point_count} = {expected_count} 點。"
            "程式仍會嘗試排序，但建議確認『單邊點位數』是否正確。"
        )

    sorted_by_y = result.sort_values("實際Y", ascending=False).reset_index(drop=True)

    rows = []
    half_index = int(point_count) // 2

    for row_idx in range(int(point_count)):
        start_idx = row_idx * int(point_count)
        end_idx = start_idx + int(point_count)
        row_df = sorted_by_y.iloc[start_idx:end_idx].copy()

        if row_df.empty:
            continue

        # 同一列內：X由小到大，對應索引X = 負到正
        row_df = row_df.sort_values("實際X", ascending=True).reset_index(drop=True)
        row_df["索引Y"] = half_index - row_idx
        row_df["索引X"] = np.arange(-half_index, -half_index + len(row_df))
        rows.append(row_df)
    ...
    if auto_fit_matrix:
        col_center = mapped.groupby("索引X")["實際X"].mean().sort_index()
        row_center = mapped.groupby("索引Y")["實際Y"].mean().sort_index()

        if len(col_center) >= 2:
            step_x = float(np.median(np.diff(col_center.values)))
        else:
            step_x = float(matrix_length) / max(int(point_count) - 1, 1)

        if len(row_center) >= 2:
            step_y = float(np.median(np.diff(row_center.values)))
        else:
            step_y = float(matrix_length) / max(int(point_count) - 1, 1)

        center_x = float(col_center.loc[0]) if 0 in col_center.index else float((mapped["實際X"].max() + mapped["實際X"].min()) / 2)
        center_y = float(row_center.loc[0]) if 0 in row_center.index else float((mapped["實際Y"].max() + mapped["實際Y"].min()) / 2)

        mapped["理論X"] = center_x + mapped["索引X"] * step_x
        mapped["理論Y"] = center_y + mapped["索引Y"] * step_y
        mapped["矩陣來源"] = "自動由實際值推估"
        mapped["推估步距X"] = step_x
        mapped["推估步距Y"] = step_y
    else:
        step = float(matrix_length) / max(int(point_count) - 1, 1)
        mapped["理論X"] = mapped["索引X"] * step
        mapped["理論Y"] = mapped["索引Y"] * step
        mapped["矩陣來源"] = "手動矩陣總長度"
        mapped["推估步距X"] = step
        mapped["推估步距Y"] = step

    # 點位名稱依照矩陣位置重新命名：左上到右下 P01 ~ P49
    mapped = mapped.sort_values(["索引Y", "索引X"], ascending=[False, True]).reset_index(drop=True)
    mapped["矩陣點位序號"] = np.arange(1, len(mapped) + 1)
    mapped["點位"] = "P" + mapped["矩陣點位序號"].astype(str).str.zfill(2)
    mapped["點位索引"] = "(" + mapped["索引X"].astype(str) + ", " + mapped["索引Y"].astype(str) + ")"

    return mapped

def parse_excel(uploaded_file, file_name, point_count, matrix_length,
                auto_detect_point_count, auto_fit_matrix,
                threshold,
                use_rule1, rule1_name, rule1_threshold, rule1_color,
                use_rule2, rule2_name, rule2_threshold, rule2_color,
                arrow_mode):
    ...
    for i in range(len(raw)):
        ...
        if "基元屬性" in row_text and "實測值" in row_text:
            header_row = i
            break

    if header_row is None:
        st.error(f"{file_name} 找不到『基元屬性 / 實測值』表頭")
        return None

    df = pd.read_excel(uploaded_file, header=header_row)

    required_cols = ["基元屬性", "實測值"]
    missing_cols = [col for col in required_cols if col not in df.columns]
    if missing_cols:
        st.error(f"{file_name} 缺少欄位：{missing_cols}")
        return None

    # 只取座標資料，只使用實測值，不使用標準值
    ...
    df["座標類型"] = df["基元屬性"].astype(str).apply(
        lambda x: "X" if "座標X" in x else ("Y" if "座標Y" in x else "")
    )

    df = df[df["座標類型"].isin(["X", "Y"])]

    x_df = df[df["座標類型"] == "X"][["點位編號", "實測值"]].rename(
        columns={"實測值": "實際X"}
    )

    y_df = df[df["座標類型"] == "Y"][["點位編號", "實測值"]].rename(
        columns={"實測值": "實際Y"}
    )
    ...
    result = result.dropna(subset=["實際X", "實際Y"])

    if result.empty:
        st.error(f"{file_name} 沒有成功解析到 X/Y 實際座標資料")
        return None

    # 可選：依實際點數自動辨識 7×7、9×9...
    if auto_detect_point_count:
        point_count = detect_square_point_count(len(result), point_count)

    # 關鍵改法：依照實際座標重建矩陣，再產生理論座標
    result = assign_matrix_by_actual_position(result, point_count, matrix_length, auto_fit_matrix)
...
auto_detect_point_count = st.sidebar.checkbox(
    "自動依點數辨識矩陣，例如49點=7×7",
    value=True
)
...
point_count = st.sidebar.number_input(
    ...
    value=7,
    step=2,
    disabled=auto_detect_point_count
)

auto_fit_matrix = st.sidebar.checkbox(
    "Excel只有實際值：自動由實際值推估理論矩陣",
    value=True
)

matrix_length = st.sidebar.number_input(
    "手動矩陣總長度 mm（關閉自動推估時使用）",
    min_value=1.0,
    value=99.0,
    step=1.0,
    disabled=auto_fit_matrix
)

st.sidebar.caption("建議：你的這份7×7資料座標約為 ±49.5 mm，所以手動矩陣總長度約 99 mm。")
...
st.sidebar.markdown(
    f'<div class="color-row"><span class="color-chip" style="background:{ABNORMAL_COLOR};"></span>|X誤差| 或 |Y誤差| ≥ {threshold:.3f} mm：異常</div>',
    unsafe_allow_html=True
)
...
# =========================
# 基礎矩陣（尚未上傳前先用手動矩陣長度顯示）
# =========================
point_count = int(point_count)
index_range = np.arange(-(point_count // 2), point_count // 2 + 1)
manual_step = matrix_length / max(point_count - 1, 1)
ideal_df = build_ideal_df(index_range, manual_step)
...
with chart_col:
    ...
    if plot_mode == "靶心圖：X/Y誤差(mm)":
        ...
        # 依照目前資料自動決定顯示範圍，避免自動推估矩陣時仍使用手動矩陣長度造成畫面過大或過小
        x_all = pd.concat([plot_df["理論X"], plot_df["顯示實測X"]])
        y_all = pd.concat([plot_df["理論Y"], plot_df["顯示實測Y"]])
        x_span = max(float(x_all.max() - x_all.min()), 1.0)
        y_span = max(float(y_all.max() - y_all.min()), 1.0)
        margin = max(x_span, y_span) * 0.08
        x_range = [float(x_all.min() - margin), float(x_all.max() + margin)]
...
# =========================
with st.expander("理論座標矩陣", expanded=False):
    if not result_df.empty and "理論X" in plot_df.columns:
        display_ideal_df = plot_df[["點位", "索引X", "索引Y", "理論X", "理論Y", "矩陣來源", "推估步距X", "推估步距Y"]].copy()
